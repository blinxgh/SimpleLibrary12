...
function Sen:DraggingEnabled(frame, parent)
...
local themes = {
	...
	ElementColor = Color3.fromRGB(32,56,78)
}
...
Setting = game:Service'HttpService':JSONEncode(readfile(Name))
end)

local LibName = tostring(math.random(1, 100))..tostring(math.random(1, 50))..tostring(math.random(1, 100))

function Sen:ToggleUI()
...
function Sen.CreateLib(senName, themeList)
   ...
   local selectedTab
   senName = senName or "Library"
   table.insert(Sen, senName)
   for i, v in pairs(game.CoreGui:GetChildren()) do
        if v:IsA("ScreenGui") and v.Name == senName then
           v:Destroy() 
        end
   end
    ...
    sen:DraggingEnabled(MainHeader, Main)
